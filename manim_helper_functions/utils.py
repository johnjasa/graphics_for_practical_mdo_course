...
def make_title_slide(scene, title, contents_list, intro_message, outro_message):
...
def make_venn(scene, types=['opt', 'mda', 'diff'], show_center_words=False):
    ...
    opt_text = Tex("Optimization").set_color(opt_color)
    ...
    if show_center_words:
        ...
        scene.wait(1)
        mdo_text = Tex("MDAO").set_color(BLACK).move_to(0.1*UP)
